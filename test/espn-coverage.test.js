...
const match1 = MATCHES.find((m) => m.num === 1) // Germany v Scotland (the opener)
...
describe('fetchLive — substitutions, unknown teams, and pens', () => {
  it('parses substitution details into per-side names', async () => {
      ...
      events: [
        {
          ...
          competitions: [
            {
              competitors: [
                { homeAway: 'home', team: { id: 'H', displayName: 'Germany' }, score: '1' },
                { homeAway: 'away', team: { id: 'A', displayName: 'Hungary' }, score: '0' },
              ],
              details: [
                {
    ...
    const rec = map.get(pairKey('Germany', 'Hungary'))
  ...
  it('skips events without two real competitors', async () => {
    const feed = {
      events: [
        // no competitions
        { date: '2024-06-14T19:00Z', status: { type: { state: 'in' } } },
        // competition without a competitors array
        { date: '2024-06-14T19:00Z', competitions: [{ competitors: null }] },
...
describe('applyLive — knockout name resolution on pre/no-score records', () => {
  it('resolves placeholder names from a pre-match ESPN record without writing a score', () => {
    ...
    const map = new Map([
      [instOf(ko), { home: 'Spain', away: 'England', score: null, state: 'pre', clock: '', detail: '' }],
    ])
    const m = applyLive(MATCHES, map).find((x) => x.num === 38)
  ...
  it('leaves a placeholder untouched when ESPN also has only placeholders', () => {
    const ko = MATCHES.find((m) => m.num === 38)
    const before = ko
    const map = new Map([
  ...
  it('leaves a real-team pre-match untouched (no score yet)', () => {
    const map = new Map([
      [pairKey('Germany', 'Scotland'), { home: 'Germany', away: 'Scotland', score: null, state: 'pre', clock: '', detail: '' }],
    ])
