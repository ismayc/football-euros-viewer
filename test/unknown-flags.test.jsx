...
describe('flag fallbacks for a team with no flag', () => {
  ...
  it('still highlights a followed candidate inside a feeder pair', () => {
    // The schedule card shows an unresolved tie as "A / B". Following one of the
    // two has to mark that half, the same as it does everywhere a team is named.
    const followed = MATCHES.find((m) => m.stage === 'Group').t1
    localStorage.setItem('euros:followed', JSON.stringify([followed]))
    try {
      render(wrap(<FeederPair feeder={{ kind: 'Winner', num: 25, a: followed, b: NO_FLAG }} />))
      const cands = [...document.querySelectorAll('.feeder-cand')]
      expect(cands.map((c) => c.className)).toEqual(['feeder-cand followed', 'feeder-cand'])
    } finally {
      localStorage.clear()
    }
  })
