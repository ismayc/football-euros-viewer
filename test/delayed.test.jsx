...
describe('Standings — delayed group', () => {
  it('shows amber "⏸ DELAYED" + delayed dots for a group whose match is paused', () => {
    // M1 = Germany v Scotland (Group A, the opener) — mark it delayed.
    const matches = MATCHES.map((m) => (m.num === 1 ? { ...m, live: { delayed: true }, score: [1, 0] } : m))
...
describe('espn — STATUS_DELAYED flows through to m.live.delayed', () => {
  ...
  // M21 = Netherlands v France (Group D), kick-off 2024-06-21T21:00+02:00.
  const event = {
    ...
    competitions: [
      {
        status: { type },
        competitors: [
          { homeAway: 'home', team: { id: '1', displayName: 'Netherlands' }, score: '1' },
          { homeAway: 'away', team: { id: '2', displayName: 'France' }, score: '0' },
        ],
      },
    ],
  }
  const fra = MATCHES.find((m) => m.num === 21)
