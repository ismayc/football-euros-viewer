...
describe('fetchBootExtras', () => {
  ...
  it('serves from the localStorage cache within the TTL', async () => {
    ...
    const c = JSON.parse(localStorage.getItem('euros:bootExtras'))
    localStorage.setItem('euros:bootExtras', JSON.stringify({ ...c, at: Date.now() - CACHE_TTL_MS - 1 }))
    await fetchBootExtras()
...
describe('fetchRecentPlayerStats', () => {
  ...
  it('trusts a cached per-match line, skips an appearance-less log item, and survives unreadable cache', async () => {
    ...
    localStorage.setItem('euros:matchStat:700:45843', JSON.stringify({ a: 9, m: 9 }))
    ...
    localStorage.setItem('euros:matchStat:700:219713', 'not json')
    const seen = []
    vi.stubGlobal('fetch', vi.fn(async (url) => {
      seen.push(url)
  ...
  it('skips a log item with no event reference and reads an empty stat line as zero', async () => {
    vi.stubGlobal('fetch', vi.fn(async (url) => {
      if (url.includes('/eventlog')) {
        const athlete = /athletes\/(\d+)\/eventlog/.exec(url)[1]
        return {
          ok: true,
          json: async () => ({
            events: {
              items: [
                // An appearance whose event link never came back: there is no id
                // to key the per-match cache on, so it contributes nothing rather
                // than being counted against an empty key.
                { ...logItem(702, athlete), event: undefined },
                logItem(703, athlete),
              ],
            },
          }),
      ...
      // …and the one appearance we can identify has a stat line with neither
      // assists nor minutes in it, which reads as zero of each, not NaN.
      if (url.includes('/statistics')) return { ok: true, json: async () => ({ splits: { categories: [] } }) }
      return { ok: true, json: async () => route(url) }
    }))
    const out = await fetchRecentPlayerStats([{ espnId: '701', live: false }], wantKeys)
    expect(out.every((e) => e.assists === 0 && e.minutes === null)).toBe(true)
