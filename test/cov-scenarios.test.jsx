...
describe('ScenariosView — teams, ties and matchups the projection cannot fill in', () => {
  ...
  // rather than to the European Qualifiers ranking.
  const withCards = () => {
    const scores = PAIRS.map(() => [0, 0])
    scores[5] = undefined
    const board = groupA(scores)
    // Germany v Switzerland: Switzerland picks up a red, so their conduct score
    // is the worse one and the pair straddling them is split on cards.
  ...
  it('says a soft tie-break was decided on cards, and warns the card data is best-effort', () => {
    ...
    expect(marks.some((m) => /disciplinary points \(cards\)/.test(m.getAttribute('title')))).toBe(true)
  ...
  it('shows TBD for a projected opponent the bracket cannot name yet', () => {
    ...
    render(<ScenariosView matches={board} />)
    const first = document.querySelector('.sc-r16-row')
    expect(first.querySelector('.sc-r16-opp').textContent).toBe('TBD')
    expect(first.querySelector('.sc-r16-num').textContent).toBe('M900')
  })
})
