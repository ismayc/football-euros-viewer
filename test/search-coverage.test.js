...
describe('matchesSearch — token fields', () => {
  ...
  it('matches a country token, including the host-country synonyms', () => {
    const hostMatch = MATCHES.find((m) => VENUES[m.venue]?.country === 'Germany')
    const country = (value) =>
      matchesSearch(hostMatch, VENUES[hostMatch.venue], { tokens: [{ field: 'country', value }], free: '' })
    expect(country('germany')).toBe(true)
    expect(country('deutschland')).toBe(true) // synonym, not a substring
    expect(country('ger')).toBe(true)
    expect(country('france')).toBe(false)
  })
