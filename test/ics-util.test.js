...
describe('buildICS (per-match calendar)', () => {
  it('wraps a single VEVENT in a VCALENDAR with UTC times', () => {
    ...
    expect(ics).toContain(`UID:euro2024-match-${groupMatch.num}@footballeurosviewer`)
  ...
  it('identifies itself as this edition, in both the single and collection feeds', () => {
    // A calendar client shows PRODID, and an inherited one would name the wrong
    // tournament in every downloaded .ics — invisible to every other assertion.
    for (const ics of [buildICS(groupMatch), buildICSCollection([groupMatch])]) {
      expect(ics).toContain('PRODID:-//Euro 2024 Viewer//EN')
      expect(ics).not.toMatch(/World Cup/)
    }
  })
...
describe('buildICSCollection (multi-match feed)', () => {
  ...
  it('defaults the calendar name', () => {
    ...
    expect(ics).toContain('X-WR-CALNAME:Euro 2024')
  })
