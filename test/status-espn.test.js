...
function event({ name, state, home = 'France', away = 'Poland', hs, as, description, shortDetail }) {
...
async function recOf(ev, home = 'France', away = 'Poland') {
...
// drives the outcome.
// M29 (France v Poland, Group D) — a real fixture, so the live overlay's
// team-name matching behaves exactly as it does in the app.
const bareMatch = (t1 = 'France', t2 = 'Poland') => ({ num: 29, stage: 'Group', group: 'D', t1, t2, ko: '2024-06-25T18:00:00+02:00' })
...
describe('applyLive — one-off status outcomes on an unscored match', () => {
  it('SUSPENDED -> m.live.delayed && m.live.label === "Suspended"', async () => {
    ...
    const map = new Map([[pairKey('France', 'Poland'), rec]])
