...
describe('liveState — voided', () => {
  it('returns "voided" for an abandoned/postponed/canceled match', () => {
    expect(liveState({ voided: true, ko: '2024-06-22T21:00:00-04:00' })).toBe('voided')
  })
})
...
// Group A fixtures: M1 GER v SCO, M2 HUN v SUI, M14 GER v HUN, M15 SCO v SUI,
// M25 SCO v HUN, M26 SUI v GER.
...
describe('clinch — voided match is not decided', () => {
  it('a voided result does not clinch the group', () => {
    // Scores that, with M1 final too, hand Germany the group on 6 pts with only
    // Switzerland v Germany left and nobody else able to reach 6. But M1
    // (Germany v Scotland) is ABANDONED — voided, so it must NOT count, leaving
    // Germany on 3 and the group still open.
    const base = {
      14: [3, 0], // Germany 3-0 Hungary (final)
      2: [0, 0], // Hungary 0-0 Switzerland (final)
      15: [0, 0], // Scotland 0-0 Switzerland (final)
      25: [0, 0], // Scotland 0-0 Hungary (final)
    }
    const decided = computeClinch(withScores({ ...base, 1: [3, 0] }))
    expect(decided['Germany']).toBe('won-group')

    // Now the same big Germany win, but flagged voided -> ignored.
    const matches = withScores(base).map((m) =>
      m.num === 1 ? { ...m, voided: true, statusLabel: 'Abandoned', score: [3, 0] } : m,
    )
    const status = computeClinch(matches)
    expect(status['Germany']).not.toBe('won-group')
  })
})
...
describe('qualification — voided match excluded from standings & completion', () => {
  it('a voided group match neither scores points/GD nor completes the group', () => {
    ...
    const target = cMatches[0] // e.g. Denmark v England fixture
