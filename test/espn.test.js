...
describe('scoreboardDates', () => {
  it('returns the UTC day before/of/after a base instant', () => {
    expect(scoreboardDates(new Date('2024-06-16T12:00:00Z'))).toEqual(['20240615', '20240616', '20240617'])
  })

  it('covers a midnight-ET kickoff under the right UTC date (the late-match lag bug)', () => {
    // Midnight ET on June 16 = 04:00Z June 16 — must include 20240616, which ESPN's
    // default slate was lagging behind.
    expect(scoreboardDates(new Date('2024-06-16T04:00:00Z'))).toContain('20240616')
  })
})
...
describe('historyDates', () => {
  it('lists distinct UTC dates of already-finished matches, excluding the live window', () => {
    ...
    const dates = historyDates(MATCHES, new Date('2024-06-18T12:00:00Z'))
  ...
  it('excludes matches that have not kicked off yet', () => {
    const dates = historyDates(MATCHES, new Date('2024-06-16T12:00:00Z'))
    // Jun 12/13/14 are in the live window; only Jun 11 is older + finished.
    expect(dates).toEqual(['20240614'])
  })
})

const match1 = MATCHES.find((m) => m.num === 1) // Germany v Scotland
...
describe('fetchLive (parsing ESPN shape)', () => {
  ...
  it('parses live score, clock, and keys by pair + instant; maps ESPN aliases', async () => {
    const feed = {
      events: [
        event({ date: '2024-06-14T19:00Z', state: 'in', clock: "67'", home: 'Germany', hs: '2', away: 'Scotland', as: '1' }),
        // ESPN alias: "Turkey" -> "Türkiye"; pre-match has no score.
        event({ date: '2024-06-16T16:00Z', state: 'pre', clock: '0\'', home: 'Turkey', hs: '0', away: 'Georgia', as: '0' }),
      ],
    }
    global.fetch = vi.fn(async () => ({ ok: true, json: async () => feed }))
    ...
    expect(map.get('inst:' + new Date('2024-06-14T19:00Z').getTime())).toBe(rec)

    // Alias resolved, and a pre-match carries a null score.
    const pre = map.get(pairKey('Türkiye', 'Georgia'))
    expect(pre.score).toBeNull()
  ...
  it('still returns a map when only one date slate responds', async () => {
    const feed = {
      events: [
        event({ date: '2024-06-16T13:00Z', state: 'in', clock: "43'", home: 'Poland', hs: '1', away: 'Netherlands', as: '0' }),
      ],
    }
    let n = 0
...
describe('applyLive (overlay onto the merged schedule)', () => {
  it('overlays a live score oriented to our team order and sets match.live', () => {
    // ESPN reports Scotland as home — our order is (Germany, Scotland).
    const map = new Map([
      [pairKey('Germany', 'Scotland'), { home: 'Scotland', away: 'Germany', score: [1, 2], state: 'in', clock: "67'", detail: '2nd Half' }],
    ])
    const merged = applyLive(MATCHES, map)
    const m = merged.find((x) => x.num === 1)
    expect(m.score).toEqual([2, 1]) // flipped to (Germany, Scotland)
  ...
  it('defers to OpenFootball on score, but still overlays ESPN cards/subs (the missing-cards bug)', () => {
    ...
    // ESPN home = Germany = our t1, so cards/subs map straight through.
  ...
  it('orients overlaid cards when ESPN home/away is the reverse of our order', () => {
    ...
    // ESPN home = Scotland (our t2): the away card belongs to Germany (our t1).
    const map = new Map([
      [
        pairKey('Germany', 'Scotland'),
        {
  ...
  it('resolves a knockout placeholder by kickoff instant and overlays its score', () => {
    const ko = MATCHES.find((m) => m.num === 37) // Round of 16, placeholder teams
    const map = new Map([
      [instOf(ko), { home: 'Spain', away: 'England', score: [1, 0], state: 'in', clock: "30'", detail: '1st Half' }],
    ])
    const merged = applyLive(MATCHES, map)
    const m = merged.find((x) => x.num === 37)
    expect(m.t1).toBe('Spain')
    expect(m.t2).toBe('England')
  ...
  it('parses cards and preserves stoppage-time minutes, oriented to our order', async () => {
    // ESPN home = Scotland (our t2), so events on team 'A' (Germany, our t1).
    const feed = {
      events: [
        event({
          date: '2024-06-14T19:00Z', state: 'in', clock: "45'+2'",
          home: 'Scotland', hs: '0', away: 'Germany', as: '1',
          details: [
  ...
  it('parses goal events and orients the scorer timeline to our team order', async () => {
    // ESPN home = Scotland (away in our order), so goals must be flipped.
    const feed = {
      events: [
        event({
          date: '2024-06-14T19:00Z', state: 'in', clock: "31'",
          home: 'Scotland', hs: '0', away: 'Germany', as: '1',
    ...
    const m = applyLive(MATCHES, map).find((x) => x.num === 1) // our order: Germany v Scotland
    expect(m.score).toEqual([1, 0])
...
describe('espnFinalScore (getter for the reconciler)', () => {
  it('returns an oriented final only once the match is post', () => {
    const inProgress = new Map([
      [pairKey('Germany', 'Scotland'), { home: 'Germany', away: 'Scotland', score: [1, 0], state: 'in' }],
    ])
    expect(espnFinalScore(match1, inProgress)).toBeNull()

    const done = new Map([
      [pairKey('Germany', 'Scotland'), { home: 'Germany', away: 'Scotland', score: [2, 1], state: 'post' }],
    ])
    expect(espnFinalScore(match1, done)).toEqual({ home: 'Germany', away: 'Scotland', ft: [2, 1] })
  })
})
