...
const m = (over) => ({ stage: 'Group', t1: 'Denmark', t2: 'Ghana', ...over })

describe('teamRecord', () => {
  const matches = [
    // Group: win, draw, loss (from Denmark's perspective; one as t2).
    m({ score: [3, 1], goals: {} }),
    m({ t1: 'Ghana', t2: 'Denmark', score: [2, 2] }),
    m({ score: [0, 1] }),
  ...
  it('aggregates W/D/L, goals, and clean sheets across finished matches', () => {
    const r = teamRecord(matches, 'Denmark')
  ...
  it('counts a shootout defeat, and orders several shootouts by kickoff', () => {
    ...
    const lost = teamRecord([m({ stage: 'QF', ko: '2026-07-04T18:00:00Z', score: [1, 1], pens: [3, 5] })], 'Denmark')
  ...
  it('orients the score when the team is t2', () => {
    const r = teamRecord([m({ t1: 'Ghana', t2: 'Denmark', score: [0, 2] })], 'Denmark')
    expect(r.w).toBe(1)
    expect(r.gf).toBe(2)
    expect(r.ga).toBe(0)
...
describe('topScorers', () => {
  ...
  it('aggregates goals per player+team, own goals excluded, penalties counted', () => {
    ...
    expect(s[0]).toMatchObject({ name: 'Vini', team: 'Denmark', goals: 3, pens: 1 })
...
describe('activeTeams', () => {
  it('collects real teams from unplayed and live matches only', () => {
    const teams = activeTeams([
      ...
      m({ stage: 'SF', t1: 'England', t2: 'Denmark', score: [1, 0], live: { minute: 70 } }), // live → active
      m({ stage: 'QF', t1: 'Ghana', t2: 'Germany', score: [2, 0] }), // final → not active
      m({ stage: 'Final', t1: 'Winner Match 101', t2: 'Winner Match 102' }), // placeholders → skipped
      m({ t1: 'Albania', t2: 'Uruguay', voided: true }), // voided → skipped
    ])
    expect([...teams].sort()).toEqual(['Denmark', 'England', 'France', 'Spain'])
  })
