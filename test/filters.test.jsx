...
describe('Filters', () => {
  ...
  it('applies a search-example chip', () => {
    render(<Harness />)
    fireEvent.click(screen.getByRole('button', { name: /Search/ }))
    fireEvent.click(screen.getByRole('button', { name: 'city: Munich' }))
    expect(screen.getByPlaceholderText(/Search — try/).value).toBe('city: Munich')
  })
  ...
  it('changes each dropdown filter', () => {
    ...
    fireEvent.change(byLabel('Host country'), { target: { value: 'Germany' } })
    expect(byLabel('Host country').value).toBe('Germany')
