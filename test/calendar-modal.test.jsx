...
function renderModal({ followedTeams = [], onClose = vi.fn() } = {}) {
  localStorage.setItem('euros:followed', JSON.stringify(followedTeams))
...
describe('CalendarModal', () => {
  ...
  it('renders the all-matches subscribe row and download buttons (no followed teams)', () => {
    ...
    expect(screen.getByText('All 51 matches')).toBeInTheDocument()
  ...
  it('downloads all matches and current filter via the ICS helper', () => {
    ...
    expect(spy).toHaveBeenCalledWith(MATCHES, 'euro-2024-all.ics')
    expect(spy).toHaveBeenCalledWith(filtered, 'euro-2024-filtered.ics')
  })
  ...
  it('downloads my-teams matches when followed', () => {
    ...
    expect(spy).toHaveBeenCalledWith(myMatches, 'euro-2024-my-teams.ics', 'Euro 2024 — My Teams')
  })
  ...
  it('copies the feed URL to the clipboard and shows "Copied!" then reverts', async () => {
    ...
    renderModal()
    const row = screen.getByText('All 51 matches').closest('.cal-row')
