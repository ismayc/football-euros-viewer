...
const match1 = MATCHES.find((m) => m.num === 1) // Germany v Hungary
...
describe('crossCheck', () => {
  ...
  it('agrees across sources even when home/away orientation differs', () => {
    const sources = [
      src('A', { home: 'Germany', away: 'Hungary', ft: [2, 1] }),
      src('B', { home: 'Hungary', away: 'Germany', ft: [1, 2] }), // flipped, same result
      src('C', { home: 'Germany', away: 'Hungary', ft: [2, 1] }),
    ]
  ...
  it('flags disagreement', () => {
    const sources = [
      src('A', { home: 'Germany', away: 'Hungary', ft: [2, 1] }),
      src('B', { home: 'Germany', away: 'Hungary', ft: [2, 2] }),
    ]
...
describe('annotateScoreChecks', () => {
  it('sets scoreCheck only on matches with a multi-source final, immutably', () => {
    const sources = [
      { name: 'A', score: (m) => (m.num === 1 ? { home: 'Germany', away: 'Hungary', ft: [2, 1] } : null) },
      { name: 'B', score: (m) => (m.num === 1 ? { home: 'Germany', away: 'Hungary', ft: [2, 1] } : null) },
    ]
...
describe('reconcileScores', () => {
  it('lists every source score for disagreeing matches only', () => {
    const agree = [
      { name: 'OpenFootball', score: (m) => (m.num === 1 ? { home: 'Germany', away: 'Hungary', ft: [2, 1] } : null) },
      { name: 'ESPN', score: (m) => (m.num === 1 ? { home: 'Germany', away: 'Hungary', ft: [2, 1] } : null) },
    ]
    ...
    const disagree = [
      { name: 'OpenFootball', score: (m) => (m.num === 1 ? { home: 'Germany', away: 'Hungary', ft: [2, 1] } : null) },
      { name: 'ESPN', score: (m) => (m.num === 1 ? { home: 'Germany', away: 'Hungary', ft: [2, 1] } : null) },
      { name: 'TheSportsDB', score: (m) => (m.num === 1 ? { home: 'Germany', away: 'Hungary', ft: [3, 1] } : null) },
    ]
