...
describe('StatsView', () => {
  ...
  it('bolds scorers whose team still has football to play', () => {
    // Add an unplayed SF for Germany → Jiménez & co. are active; the others frozen.
    const withRemaining = [...matches, { num: 101, stage: 'SF', t1: 'Germany', t2: 'France' }]
  ...
  it('overrides the lagging aggregate assists + minutes for a live match', async () => {
    ...
    const live = [
      ...matches,
      { num: 101, stage: 'SF', t1: 'Germany', t2: 'France', score: [0, 0], live: { minute: 80 }, espnId: '999', ko: recentKo(), goals: { t1: [], t2: [] } },
    ]
  ...
  it('marks players whose team is in a live match with an in-action dot', () => {
    const live = [...matches, { num: 101, stage: 'SF', t1: 'Germany', t2: 'France', score: [0, 0], live: { minute: 10 }, goals: { t1: [], t2: [] } }]
    render(<StatsView matches={live} hideScores={false} />)
    const rows = screen.getAllByRole('row').slice(1)
    const jimenez = rows.find((r) => r.textContent.includes('Raúl Jiménez'))
    const son = rows.find((r) => r.textContent.includes('Son Heung-min'))
    expect(jimenez.querySelector('.boot-live')).toBeTruthy() // Germany are playing now
...
describe('StatsView tile drill-down', () => {
  const knockout = [
    ...
    { num: 97, stage: 'QF', t1: 'Spain', t2: 'Denmark', ko: '2024-07-10T15:00:00-04:00', score: [1, 1], aet: true, pens: [4, 2] },
  ]
  ...
  it('expands the extra-time tile into its match list (shootout noted)', () => {
    ...
    expect(screen.getByText(/Spain 1–1 Denmark/)).toBeInTheDocument()
  ...
  it('expands shootouts separately, and a row opens the match detail', () => {
    ...
    fireEvent.click(screen.getByText(/Spain 1–1 Denmark/))
...
describe('StatsView — enrichment that lands late, lands empty, or names a stranger', () => {
  const liveBoard = () => [
    ...matches,
    { num: 61, stage: 'SF', t1: 'Germany', t2: 'Albania', score: [0, 0], live: { minute: 10 }, goals: { t1: [], t2: [] } },
  ]
  ...
  it('folds an interval refresh that finally returns something into the table', async () => {
    ...
    try {
      ...
      fetchBootExtras.mockImplementation(async () => [
        { name: 'Raúl Jiménez', goals: 3, assists: 2, minutes: 180 },
        { name: 'Son Heung-min', goals: 1, assists: 0, minutes: 90 },
      ])
      await act(async () => {
        await vi.advanceTimersByTimeAsync(5 * 60 * 1000 + 1000)
      })
  ...
  it('treats an empty reconciliation as no override at all', async () => {
    ...
    const recent = [
      ...matches,
      { num: 62, stage: 'SF', t1: 'Germany', t2: 'Albania', ko: recentKo(), score: [1, 0], espnId: 'e62', goals: { t1: [{ name: 'Raúl Jiménez' }], t2: [] } },
    ]
    render(<StatsView matches={recent} hideScores={false} />)
    await vi.waitFor(() => expect(fetchRecentPlayerStats).toHaveBeenCalled())
    const row = screen.getByText('Raúl Jiménez').closest('tr')
