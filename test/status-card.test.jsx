...
const base = MATCHES.find((m) => m.num === 1) // Germany v Scotland (Group A)
const knockout = MATCHES.find((m) => m.stage === 'R16')
...
describe('MatchCard normal states (full coverage)', () => {
  ...
  it('past kickoff with no live feed → Delayed (not LIVE)', () => {
    vi.useFakeTimers()
    try {
...
describe('MatchCard follow / clinch / slot tooltip', () => {
  it('toggles follow', () => {
    renderCard()
    fireEvent.click(screen.getByRole('button', { name: 'Follow Germany' }))
    expect(screen.getByRole('button', { name: 'Unfollow Germany' })).toBeInTheDocument()
  })

  it('won-group clinch badge + slot tooltip', () => {
    renderCard(base, { clinch: { Germany: 'won-group' } })
    expect(screen.getByText(/Won group/)).toBeInTheDocument()
    expect(screen.getByText('Germany').getAttribute('title')).toMatch(/Clinched Group A winner/)
  })

  it('eliminated slot tooltip', () => {
    renderCard(base, { clinch: { Germany: 'eliminated' } })
    expect(screen.getByText('Germany').getAttribute('title')).toMatch(/Eliminated from Group A/)
  })
