...
// breakers resolve with real proportions — bubble cases on the best-thirds cut get
...
// goal difference, goals, discipline (cards), then the qualifiers ranking. Descending (< 0
// when `a` outranks `b`).
const compareThirds = (a, b) =>
  b.Pts - a.Pts || b.GD - a.GD || b.GF - a.GF || b.conduct - a.conduct || byQualifierRank(a.team, b.team)
...
// Enumerate all distinct per-group outcome combinations and tally each R16 slot,
...
export function enumerateOutlook(matches, onProgress, fixedCap) {
  ...
  const R = {}
  const T = {}
  const thirds = new Array(GROUPS.length)
  let done = 0
  const STEP = 50000
  ...
  for (;;) {
    ...
    // The best ADVANCING_THIRDS groups, as the sorted-letter key into UEFA's
    // combination table. Slicing the wrong number here silently yields a key
    // that isn't in the table, so every third-place slot comes back empty.
