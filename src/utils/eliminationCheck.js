...
// R16 Outlook's probability view, which deliberately can't answer this.
//
...
// though it is NOT eliminated (a third on 3 pts with a negative GD is the
// textbook case — Euro 2024's Slovenia advanced from exactly that position).
//
...
// the FULL UEFA third-place comparator (points → GD → goals → discipline →
...
import { entryMatches, slotLabels, WINNER_GROUP } from './slots.js'
...
// the official UEFA order: points → goal difference → goals → conduct (cards) →
// the qualifiers ranking. (No cross-group head-to-head — they never met.) Built from
...
// completion of the remaining games in which the team reaches the Round of 16
// (as a top-two finisher or one of the four best thirds). Exact when every group is
...
export function eliminationStatus(matches) {
  ...
  for (const g of GROUPS) {
    ...
    for (const name of r.names) {
      ...
      // of thirds above us. ≤3 above ⇒ we make the top 4.
...
// Convenience: the set of teams that can still reach the Round of 16 (everyone
...
// winner-group letter -> entry-round match number, for the matches that pair a
...
const WINNER_THIRD_MATCH = (() => {
  ...
  for (const m of entryMatches(MATCHES)) {
    const labels = slotLabels(m)
    const w = labels.find((s) => WINNER_GROUP.test(s))
    const third = labels.find((s) => /^3rd /.test(s))
    if (w && third) out[WINNER_GROUP.exec(w)[1]] = m.num
  }
  return out
})()

// The Round-of-16 matches a still-alive third from `team`'s group could land in,
// across every still-reachable "4 best thirds" combination (UEFA's table). A
// third's slot isn't fixed — which group winner it faces depends on which four
...
export function thirdPlaceEntrySlots(matches, team, reachable = null) {
...
export function aliveEntrySlots(matches, teams) {
  ...
  for (const t of teams) {
    ...
    const slots = thirdPlaceEntrySlots(matches, t, reachable)
...
export function advancementRequirements(matches, team, reach = null) {
  ...
  const budget = ADVANCING_THIRDS - 1 - forcedAbove // = 3 - forcedAbove
