...
export function buildICS(match) {
  ...
  const lines = [
    ...
    'CALSCALE:GREGORIAN',
    'BEGIN:VEVENT',
    `UID:euro2024-match-${match.num}@footballeurosviewer`,
...
export function downloadICS(match) {
  downloadText(buildICS(match), `euro2024-match-${match.num}.ics`)
}

// A whole calendar of matches (used by the "download all / my teams / filtered" buttons).
export function buildICSCollection(matches, calName = 'Euro 2024') {
  return [
    'BEGIN:VCALENDAR',
    'VERSION:2.0',
    'PRODID:-//Euro 2024 Viewer//EN',
...
export function downloadICSCollection(matches, filename = 'euro-2024.ics', calName = 'Euro 2024') {
