...
// team — using the exact UEFA tie-breakers in qualification.js (points → goal
// difference → goals scored → head-to-head). The cross-group "4 best third-
...
//   'third'     — guaranteed to advance as one of the four best third-placed teams
...
// cross-group "4 best thirds" combinations are still reachable. Returns
...
export function clinchHeadline({ team, group, status }) {
  switch (status) {
    ...
    case 'third':
      return `✅ ${team} are THROUGH to the Round of 16 (Group ${group})`
    case 'eliminated':
...
export function clinchBadge(status) {
  switch (status) {
    ...
    case 'third':
      return { cls: 'c-in', label: '✅', text: 'Through (3rd)', title: 'Has clinched advancement to the Round of 16 as one of the four best third-placed teams' }
    case 'eliminated':
