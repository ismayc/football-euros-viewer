...
// Host-country synonyms. Every Euro 2024 ground is in Germany, so this is really
// one entry — but a reader searching in German types "Deutschland", which the
// plain substring test would miss.
const COUNTRY_SYN = { germany: ['deutschland', 'ger', 'de'] }

function matchCountry(country, v) {
  const c = country.toLowerCase()
  if (c.includes(v)) return true
  if (COUNTRY_SYN[c]?.includes(v)) return true
  return false
}
