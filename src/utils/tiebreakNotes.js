// Detects when a group placing came down to a "soft" tie-breaker — disciplinary
// points (cards) or the European Qualifiers ranking — i.e. two teams were level
// through points, head-to-head, overall goal difference, AND goals scored, and
// only criteria 7 (discipline) or 8 (ranking) separated them.
//
// Mirrors the exact clustering in qualification.js's resolveLevelOnPoints: the
// only place those two decide order is the final branch, where a head-to-head
// cluster that couldn't be separated is sorted by overall GD, GF, then
// discipline, then ranking. Two teams equal on GD and GF in that branch were
// therefore split by one of the two.
...
// GD/GF couldn't separate (so discipline or the ranking decided the order).
function markCluster(tied, group, matches, notes) {
  ...
  while (i < sorted.length) {
    ...
    if (cluster.length > 1 && cluster.length < tied.length) {
      ...
      // Couldn't separate on head-to-head → overall GD, GF, discipline, ranking.
      ...
      for (let k = 0; k + 1 < ord.length; k++) {
        ...
        if (a.GD === b.GD && a.GF === b.GF) {
          const reason = a.conduct !== b.conduct ? 'conduct' : 'ranking'
...
// Map of team name -> { reason: 'conduct' | 'ranking', vs: otherTeamName } for any
// team separated from an adjacent team only by discipline or the ranking.
...
// teams never met), so it's ranked by points, GD, goals, then discipline, then
// the ranking. Flag any third separated from an adjacent third only by those.
...
export const TIEBREAK_LABEL = {
  conduct: 'disciplinary points (cards)',
  ranking: 'European Qualifiers ranking',
}
