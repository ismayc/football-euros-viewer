...
// Round-of-16 tie across every still-reachable outcome:
//   • Winner / Runner-up slot  -> locked iff that group's winner/runner-up is
//     itself clinched (a single team).
//   • Third-place slot          -> enumerate the still-reachable "4 best thirds"
//     combinations (UEFA's table); if every one assigns this winner the SAME
...
// An entry-round slot label is always one of exactly three shapes, so the return
...
// Which "4 best third-placed groups" combinations are STILL reachable. For each
// candidate (the keys of the UEFA table), set in-combination groups to their
...
export function reachableThirdSets(matches) {
  ...
  for (const key of Object.keys(THIRD_PLACE_COMBINATIONS)) {
    ...
    /* v8 ignore next -- unreachable: every key of the UEFA table is four real group letters, so the loop above always sets a weakest included profile */
...
// The locked Round-of-16 opponent for `team`, or null if it isn't mathematically fixed
...
export function lockedOpponent(matches, team, clinch = computeClinch(matches), reachable = null) {
  ...
  // Everything left is a third-place slot, and only the four winner-vs-third ties
  // reach here — so `group` is always one of the hosts in THIRD_WINNER_ORDER.
