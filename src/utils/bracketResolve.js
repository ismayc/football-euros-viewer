...
//   • third-place qualifiers fill the "3rd Group X/Y/Z" Round-of-16 slots once
//     the group stage is fully FINAL (which of the six thirds advance, and the
//     UEFA-table tie they each land in, is only settled then), reusing the
//     projection engine in asItStands.js;
...
// Static entry-round slots (invariant labels), for resolving third-place ties.
const ENTRY_STATIC = entryMatches(MATCHES)
...
// Fill the "3rd Group X/Y/Z" Round-of-16 slots with the actual third-placed
// teams. Gated on a fully-final group stage: which third lands where depends on
// which four of the six thirds advance (UEFA's combination table), and that's
// only settled once every group is done. Until then the Standings "as it stands"
// panel shows the provisional projection.
export function resolveThirdPlaceSlots(matches) {
  ...
  const fill = {} // entry-round match number -> third-placed team
  ...
  return matches.map((m) => {
    ...
    if (!team) return m
    const t1 = THIRD_GROUP.test(m.t1) ? team : m.t1
    const t2 = THIRD_GROUP.test(m.t2) ? team : m.t2
    return t1 === m.t1 && t2 === m.t2 ? m : { ...m, t1, t2 }
  })
}

// Fill a "3rd Group X/Y/Z" slot the moment its team is mathematically LOCKED —
// even before the whole group stage is final, and independent of whether that
// slot's WINNER side is decided yet. A third is locked when every still-reachable
// "4 best thirds" combination (see opponentClinch) assigns the SAME group to this
// slot's winner, and that group has finished (so its third is a single team). So
// a tie shows on the bracket as soon as it's settled, not at the last game.
