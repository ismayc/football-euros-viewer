...
// Tooltip describing which Round-of-16 slot this team feeds into, given its
...
function slotTooltip(group, slot, clinch) {
  ...
  const r16 = (num) => `Round of 16 · Match ${num}`
  if (clinch === 'won-group') return `Clinched Group ${group} winner → ${r16(slot.win)}`
  if (clinch === 'runner-up') return `Clinched Group ${group} runner-up → ${r16(slot.runnerUp)}`
  if (clinch === 'eliminated') return `Eliminated from Group ${group} — no knockout slot`
  const parts = []
  if (slot.win) parts.push(`1st → ${r16(slot.win)}`)
  if (slot.runnerUp) parts.push(`2nd → ${r16(slot.runnerUp)}`)
