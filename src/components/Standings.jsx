...
import { computeQualification, rowStatus, ADVANCING_THIRDS } from '../utils/qualification.js'
...
import { QUALIFIER_RANK } from '../data/qualifierRanking.js'
...
const STATUS_BADGE = {
  ...
  best3: { cls: 'q-best3', label: 'Provisional 3rd', title: 'Provisionally among the four best third-placed teams — NOT clinched; depends on the other groups still to finish' },
  out3: { cls: 'q-out', label: '·', title: 'Third place, outside the best four so far' },
...
// the Round of 16. A provisional snapshot — opponents shift as other groups play.
...
// the qualifiers ranking). Shows both teams' deciding values so e.g. one team above another
...
function ThirdTieNotes({ thirds }) {
  ...
  return (
    <div className="thirds-tie-note">
      ...
      <ul>
        {pairs.map(([a, b]) => {
          // discipline decides before the ranking; they're only separated by rank if
              ...
              {byConduct ? (
                <>
                  ...
                  <span className="thirds-tie-fine">
                    — closer to 0 is cleaner (−1 per yellow, −3 per red; best-effort card data)
                  </span>
                </>
              ) : (
                <>
                  <strong>European Qualifiers ranking</strong>: {a.name} #{QUALIFIER_RANK[a.name] ?? '—'} vs {b.name} #
                  {QUALIFIER_RANK[b.name] ?? '—'}{' '}
  ...
  return (
    <div className="thirds-card">
      ...
      <p className="thirds-note">
        The four best of the six third-placed teams advance (ranked by points, then goal difference,
        then goals scored, then discipline, then the European Qualifiers ranking).{' '}
        {allFinal ? (
          ''
        ) : (
          <>
      ...
      <table className="standings-table">
        ...
        <tbody>
          {qual.thirds.map((r, i) => {
            ...
            // is still provisional: the qualifying places are on the bubble
            // (yellow), the rest merely outside it for now (plain, not red).
            // Once every group is done, clinch resolves every third to 'third'
            // or 'eliminated', so this naturally lands on green for the four who
            // go through and red for the two who don't.
            const c = clinch?.[r.name]
            const rowCls = c === 'third' ? 'qualifies' : c === 'eliminated' ? 'eliminated' : i < ADVANCING_THIRDS ? 'provisional' : ''
            return (
            <tr key={r.name} className={rowCls}>
              <td className="col-team">
...
export default function Standings({ matches, tz, hideScores, clinch, onGoToMatch }) {
  ...
  // "As it stands" R16 projection is shown by default; this toggle (persisted)
    ...
    <>
      ...
      <p className="standings-legend">
        ...
        <span
          ...
          tabIndex={0}
          role="note"
          aria-label="Tie-breakers: points, then head-to-head, then goal difference, then goals, then disciplinary points (cards), then European Qualifiers ranking"
          data-tip="Tie-breakers: points → head-to-head → goal difference → goals → discipline (cards) → European Qualifiers ranking"
        >
          tie-breakers
        </span>{' '}
