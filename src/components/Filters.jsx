...
// Derived from the venue data rather than hard-coded, so a new edition's hosts
// and regions can't silently desync from the grounds actually being used — the
// failure mode being a dropdown option that matches nothing.
const uniqueSorted = (key) =>
  [...new Set(Object.values(VENUES).map((v) => v[key]).filter(Boolean))].sort((a, b) => a.localeCompare(b))
const COUNTRIES = uniqueSorted('country')
const REGIONS = uniqueSorted('region')

// One-click example queries that demonstrate the scoped-search syntax.
const SEARCH_EXAMPLES = ['team: Germany', 'city: Munich', 'stage: Final', 'group: C']
        ...
        {searchOpen ? (
          <>
            <input
              ...
              type="search"
              autoFocus
              placeholder='Search — try "team: Spain" or "city: Berlin"'
