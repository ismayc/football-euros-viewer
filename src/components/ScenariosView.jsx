...
// "As it stands → R16" line for a projected qualifier. `confirmed` is true once
...
function EntryLine({ label, dest, confirmed }) {
  ...
  return (
    <li className={`sc-r16-row${confirmed ? ' sc-r16-confirmed' : ''}`}>
      <span className="sc-r16-pos">{label}</span>
      ...
      <span className="sc-r16-team">{FLAG_BY_TEAM[dest.team]} {dest.team}</span>
      <span className="sc-r16-vs">vs</span>
      <span className="sc-r16-opp">
        {dest.opponent ? `${FLAG_BY_TEAM[dest.opponent]} ${dest.opponent}` : 'TBD'}
      </span>
      {dest.matchNum && <span className="sc-r16-num">M{dest.matchNum}</span>}
      {confirmed && <span className="sc-r16-lock" title="This matchup is confirmed — it can no longer change" aria-label="Matchup confirmed">✔️</span>}
    </li>
  )
}
    ...
    <div className="scenarios">
      <div className="sc-intro">
        <p>
          ...
          Round of 16 would shake out. Deterministic — no predictions, just the consequences of the
          results you choose.
        </p>
      ...
      <div className="sc-grid">
        {groupsInPlay.map((g) => {
          ...
          // Placings decided by a soft tie-breaker (cards / qualifiers ranking).
          ...
          return (
            <div className="sc-card" key={g}>
              ...
              <div className="sc-r16">
                <div className="sc-r16-title">Projected Round of 16</div>
                <ul className="sc-r16-list">
      ...
      <p className="sc-foot">
        ...
        points (cards) or the European Qualifiers ranking — once points, head-to-head, goal difference and goals are
