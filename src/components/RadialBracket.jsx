...
import { slotLabels } from '../utils/slots.js'
...
// A circular ("radial") rendering of the knockout bracket: the 16 Round-of-16
...
// the spokes as results land. There is no third-place play-off at the Euro.
...
const CX = 500
const CY = 500
const LEAF_R = 470 // outer ring: the 16 Round-of-16 teams
const RING = { R16: 360, QF: 240, SF: 120 } // winner of each round, inward
const SLOTS = 16

// Static feeder map: a knockout match's two source matches, parsed from the
// original "Winner/Loser Match N" labels — which a PLAYED match keeps in
// label1/label2, since t1/t2 then hold the real teams. Reading t1/t2 here would
// work only for an unplayed bracket.
...
// The 16 outer slots, clockwise from just right of top: the right half of the
...
const LEAF_SLOTS = [
  ...BRACKET.right.R16.flatMap((n) => [{ n, side: 0 }, { n, side: 1 }]),
  ...[...BRACKET.left.R16].reverse().flatMap((n) => [{ n, side: 1 }, { n, side: 0 }]),
]
const slotAngle = (i) => 90 - (i + 0.5) * (360 / SLOTS)

// Angle of every match node: an R16 node is the midpoint of its two outer slots;
...
function buildAngles() {
  ...
  for (const n of [...BRACKET.left.R16, ...BRACKET.right.R16]) {
...
const ROUND_RING_LABELS = [
  { text: 'ROUND OF 16', r: (LEAF_R + RING.R16) / 2, size: 10 },
  { text: 'QUARTER-FINALS', r: (RING.R16 + RING.QF) / 2, size: 9.5 },
...
export default function RadialBracket({ matches, tz, hideScores }) {
  ...
  // R16 winners join their two outer teams.
  for (const n of [...BRACKET.left.R16, ...BRACKET.right.R16]) {
    addBracket(n, RING.R16, LEAF_R, leafAngleOf[`${n}:0`], leafAngleOf[`${n}:1`], (RING.R16 + LEAF_R) / 2)
  }
  // Each later round joins the two it feeds from, at that round's ring.
  const CHILD_RING = { QF: RING.R16, SF: RING.QF }
  for (const round of ['QF', 'SF']) {
    for (const n of [...BRACKET.left[round], ...BRACKET.right[round]]) {
      const [c1, c2] = childMatchNums(n)
  ...
  // pinned ABOVE the trophy, where there is clear space either side of the seam.
  ...
  return (
    <div className={`radial-wrap${hasPath ? ' has-path' : ''}`}>
      ...
      <svg className="rb-svg" viewBox="0 0 1000 1080" role="img" aria-label="Knockout bracket, circular view">
        ...
        {/* Outer ring: the 16 Round-of-16 teams. */}
        {LEAF_SLOTS.map((s, i) => {
          const [x, y] = leafPos(s)
      ...
      <p className="rb-hint">
        Outer ring = Round of 16. Each match’s winner advances one ring inward toward the trophy;
