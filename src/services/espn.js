...
//
// Scores are cross-checked against OpenFootball and TheSportsDB — see
// services/reconcile.js, which flags disagreements via each source's
// *FinalScore getter (espnFinalScore is exported below).

import { normalizeTeam, isRealTeam, pairKey } from './results.js'

export const LIVE_SOURCE = {
  name: 'ESPN',
  url: 'https://site.web.api.espn.com/apis/site/v2/sports/soccer/uefa.euro/scoreboard',
  homepage: 'https://www.espn.com/soccer/',
}

// ESPN spellings that differ from ours, applied before normalizeTeam (which
// already handles the OpenFootball-style aliases "Turkey" -> "Türkiye" and
// "Czech Republic" -> "Czechia").
//
// Empty for Euro 2024: ESPN's scoreboard names all 24 sides exactly as we do,
// Türkiye and Czechia included — verified against every one of the 51 matches
// and pinned by test/fixtures/espn-team-names.json. The seam stays because an
// unmapped spelling is silent (the lookup returns a non-team and the live score
// is dropped), so a new edition's feed needs re-capturing, not trusting.
...
// dates=20260728 returns instants up to 07-29T02:00Z). Euro 2024's CEST kickoffs
// never land inside the 00:00–04:00Z window, so the UTC day happened to coincide —
// but the family convention (world-cup / copa / wwc, fixed 2026-08-09) files by the
// Eastern day, and so does this app now.
