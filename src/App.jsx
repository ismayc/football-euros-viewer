...
const GOAL_ALERTS_KEY = 'euros:goalAlerts'
...
export default function App() {
  ...
  const toggleTheme = () =>
    setTheme((t) => {
      ...
      try {
        localStorage.setItem('euros:theme', next)
      } catch {
        /* ignore */
      }
      return next
  ...
  // The group-stage tools (Scenarios, R16 Outlook) drop out of the nav a day
  ...
  // `filtered`). Applies as each round finishes (group stage, then Round of 16, …).
    ...
    <div className={`app${navAway ? ' nav-away' : ''}`}>
      <header className="app-header">
        <div className="title-block">
          <h1>
            <span className="trophy">🏆</span> Euro 2024
          </h1>
          <p className="subtitle">
            All 51 matches · Germany · shown in{' '}
      ...
      <div className={`results-bar results-${resultsState}`}>
        ...
        <span className="results-text">
          ...
          {resultsState === 'ok' && finishedCount === 0 && 'No results yet — kickoff is June 14, 2024'}
        </span>
        {liveCount > 0 && (
      ...
      <footer className="app-footer">
        <p>
          Kickoff times convert automatically to your selected timezone; they were published in
          CEST, the host timezone. Broadcast info is for the United States. Euro 2024 finished on
          14 July 2024 — this is a completed edition, kept here so the next one can slot straight
          in. Filters, timezone &amp; view are saved to the URL — bookmark or share it.
        </p>
        <p className="disclaimer">
          An unofficial fan-made project. Not affiliated with, endorsed by, or sponsored by UEFA.
          “EURO”, team, broadcaster, and tournament names are trademarks of their respective
        ...
        <p className="credit">
          ...
          ·{' '}
          <a
            href="https://github.com/ismayc/football-euros-viewer"
            target="_blank"
