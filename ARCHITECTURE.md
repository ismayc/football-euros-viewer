...
A contributor's map of the Euro 2024 Schedule Viewer. The README covers
...
| File | Exports | Role |
| --- | --- | --- |
| `matches.js` | `MATCHES`, `STAGE_LABELS` | The frozen 51-match schedule (kickoffs, venues, group/knockout slot labels, and — this edition being complete — every result). Source of truth for structure; regenerated by `fetch:tournament` and cross-checked against `test/fixtures/official-kickoffs.js`. |
...
## `scripts` — data generation & CI guards
...
`test/scripts-runtime.test.js` — so they run in CI without `npm ci`.

- **Data generation** — `fetch-tournament.mjs`: rebuild `src/data/{matches,teams,
  venues}.js` from ESPN (structure, ids, kickoffs, scores) + OpenFootball (goal
  scorers), failing if the two disagree on any score. `npm run fetch:tournament`.
- **Kickoff fixture** — `make-official-fixture.mjs`: rebuild
  `test/fixtures/official-kickoffs.js` from OpenFootball *only*, deliberately a
  different source from the generator's, so the comparison in `data.test.js` is a
  real cross-check. `npm run fixture:official`.
- **Bracket guard** — `check-bracket-consistency.mjs`: our knockout slot
  references vs the official draw. `npm run check:bracket`.
- **Badge** — `coverage-badge.mjs`: turn the coverage summary into the shields.io
  endpoint JSON served from `public/`.

The sibling `world-cup-viewer` also carries a live-tournament monitoring suite
(FIFA schedule-drift checks, OpenFootball score push-back, feed-freshness). None
of it is here: it validates a schedule that can still move against a governing
body's live calendar API, and a completed edition cannot drift.
...
- `test/` — Vitest suite (units + jsdom component tests). Fixtures in
  ...
  `final-group-results.js`, `group-stage-md3.js`, the two captured feed
  team-name lists, …) as regression anchors.
...
- **Canonical names.** Every feed normalises to the `src/data/teams.js` spelling
  at ingestion; aliases live next to each feed (`normEspn`, `normSdb`).
